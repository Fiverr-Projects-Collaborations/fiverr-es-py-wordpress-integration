...
class Post extends Indexable {
	...
	public function prepare_document( $post_id ) {
        ...
        if($post->post_type=='piatto'){
            ...
            global $wpdb;
            $results = $wpdb->get_results($wpdb->prepare('with temp as (select p.id,p.post_title as restaurant_title,pm.meta_key, pm.meta_value , wt.name as city from wp_posts p join wp_postmeta pm on p.ID = pm.post_id left join wp_term_relationships wtr on wtr.object_id = pm.post_id left join wp_terms wt on wt.term_id= wtr.term_taxonomy_id where p.post_type = \'ristorante\' and p.post_status = \'publish\' and pm.meta_key in( \'nome_ristorante_semplice\',\'restaurant_address\') and p.id = '.(int)$restaurantId.') select x.restaurant_title, x.meta_value as restaurant_name, y.meta_value as restaurant_address, x.city from temp x join temp y on x.id = y.id and x.meta_key = \'nome_ristorante_semplice\' and y.meta_key = \'restaurant_address\''),ARRAY_A);
	...
	public function format_args( $args, $wp_query ) {
                    ...
                    array(
                        'multi_match' => array(
                            ...
                            'fuzziness' => apply_filters( 'ep_fuzziness_arg', 1, $search_fields, $args ),
                                  ...
                                  array(
                                       'wildcard' => array(
                                           'meta.food_ingredients.value'     => '',
                                      ),
                                  ),
                                  array(
                                       'wildcard' => array(
                                           'meta.food_keywords.value'     => '',
                                      ),
                                  ),
                                  array(
                                       'wildcard' => array(
        ...
        if ( ! empty( $args['s'] ) ) {
            ...
            //anegi - new filter
            $query['bool']['should'][3]['wildcard']['meta.food_ingredients.value'] = '*'.$args['s'].'*';
            $query['bool']['should'][4]['wildcard']['meta.food_keywords.value'] = '*'.$args['s'].'*';
            $query['bool']['should'][5]['wildcard']['post_title'] = '*'.$args['s'].'*';
            $query['bool']['should'][6]['wildcard']['restaurant_title'] = '*'.$args['s'].'*';
            $query['bool']['should'][7]['wildcard']['restaurant_name'] = '*'.$args['s'].'*';
            $query['bool']['should'][8]['wildcard']['restaurant_address'] = '*'.$args['s'].'*';
            $query['bool']['should'][9]['wildcard']['city'] = '*'.$args['s'].'*';
            $query['bool']['should'][10]['match']['synonym_fields'] = $args['s'];
            //anegi - end of new filter
